...
def classify_stage(signal: str, long_score: int, short_score: int, blocked: bool):
    if signal == "LONG": return "ACTIVE_LONG"
    if signal == "SHORT": return "ACTIVE_SHORT"
    if blocked: return "WAIT_BLOCKED"
    if long_score >= 55 and long_score >= short_score + 12: return "WATCH_LONG"
    if short_score >= 55 and short_score >= long_score + 12: return "WATCH_SHORT"
    return "WAIT"
...
def analyze():
    ...
    stage=classify_stage(signal,ls,ss,blocked)

    # For watch stage use the dominant direction for time analogues.
    ...
    return Result(
        ...
        regime=regime, stage=stage, long_score=ls, short_score=ss, signal=signal,
