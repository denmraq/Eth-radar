...
SESSION.headers.update({"User-Agent": "ETH-Entry-Radar/0.3.3"})
...
def classify_bias(long_score: int, short_score: int):
    """Direction is shown even when entry timing is not confirmed.
    A meaningful score gap is enough for a directional bias; it is NOT an entry.
    """
    gap = long_score - short_score
    if long_score >= 25 and gap >= 15:
        return "LONG"
    if short_score >= 25 and gap <= -15:
        return "SHORT"
    return "NEUTRAL"

def classify_stage(signal: str, bias: str, long_score: int, short_score: int, blocked: bool,
                   block_long: bool=False, block_short: bool=False):
    if signal == "LONG": return "ACTIVE_LONG"
    if signal == "SHORT": return "ACTIVE_SHORT"
    if blocked: return "WAIT_BLOCKED"
    # WATCH means direction exists, but entry trigger / risk checks are not complete yet.
    if bias == "LONG" and not block_long: return "WATCH_LONG"
    if bias == "SHORT" and not block_short: return "WATCH_SHORT"
    return "WAIT"

def estimate_entry_window(stage: str, atr_distance: float):
    if stage in ("ACTIVE_LONG","ACTIVE_SHORT"):
        return "NOW–15m"
    if stage in ("WATCH_LONG","WATCH_SHORT"):
        if atr_distance <= 0.45: return "15–45m"
        if atr_distance <= 0.9: return "30–75m"
        return "45–120m"
    return None
...
def analyze():
    ...
    signal="WAIT"
    if ls>=70 and ss<=40 and not block_long:
        signal="LONG"
    elif ss>=70 and ls<=40 and not block_short:
        signal="SHORT"
    ...
    if signal in ("LONG","SHORT"):
        ...
        if signal=="LONG":
            entry_low=price-.15*atr15; entry_high=price+.10*atr15
            stop=min(rlw-.15*atr15, price-.75*atr15)
            risk=price-stop
            tp1=price+risk; tp2=price+1.7*risk; tp3=price+2.5*risk
        else:
            entry_low=price-.10*atr15; entry_high=price+.15*atr15
            stop=max(rh+.15*atr15, price+.75*atr15)
            risk=stop-price
            tp1=price-risk; tp2=price-1.7*risk; tp3=price-2.5*risk
        if risk<=0:
            signal="WAIT"; veto.append("BLOCK_BOTH: invalid structural risk.")
        else:
            rr=abs(tp2-price)/risk
            stop_atr=risk/atr15
            target_atr=abs(tp2-price)/atr15
            if risk/price>.025:
                signal="WAIT"; veto.append("BLOCK_BOTH: structural stop exceeds 2.5% of price.")
            elif rr<1.5:
                signal="WAIT"; veto.append(f"BLOCK_BOTH: R:R {rr:.2f} is below 1.5.")

    blocked = any(v.startswith("BLOCK_BOTH") for v in veto)
    bias = classify_bias(ls, ss)
    stage=classify_stage(signal,bias,ls,ss,blocked,block_long,block_short)
    entry_status = "ACTIVE" if stage.startswith("ACTIVE_") else "WATCH" if stage.startswith("WATCH_") else "WAIT"

    # For WATCH we still calculate timing statistics in the dominant direction.
    direction = "LONG" if stage in ("ACTIVE_LONG","WATCH_LONG") else "SHORT" if stage in ("ACTIVE_SHORT","WATCH_SHORT") else None
    entry_window=estimate_entry_window(stage,live_dist_atr)
