...
app = FastAPI(title="ETH Entry Radar iOS V0.3.7")

@app.middleware("http")
async def no_stale_ui_cache(request: Request, call_next):
    response = await call_next(request)
    if request.url.path == "/" or request.url.path.startswith("/api/") or request.url.path in {
...
@app.get("/api/health")
def health():
    return {"ok": True, "version": "ios-0.3.7"}
